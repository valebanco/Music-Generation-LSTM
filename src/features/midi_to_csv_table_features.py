...
def main():
	...
	for file in os.listdir(input_dir):
		...
		mf.read()
		mf.close()
		s = music21.midi.translate.midiFileToStream(mf, quantizePost=True).flat #quantize is what rounds all note durations to real music note types, not needed for our application
		...
		for note in s.recurse().notes: 
			if note.isNote:
				# print(note.offset)
				# print(note.duration)
				new_df = pd.DataFrame([[note.pitch, round(float(note.offset - offsetBase), 3), round(note.duration.quarterLength, 3), note.volume.velocity, note_tempo]], columns=["note_name", "offset", "duration", "velocity", "tempo"])
				
				df = df.append(new_df, ignore_index=True)
				
				offsetBase = float(note.offset)
				#print(df)
				#print(new_df)
				#print(note)
